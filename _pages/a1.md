...
> CMakeLists.txt is incomplete. Before `make install`, you need to configure the `db` executable by yourself (See hint in CMakeLists.txt).
...
This [System Performance Tools: perf](http://daslab.seas.harvard.edu/classes/cs165/doc/sections/S8_perf.pdf) provides a good reference for profiling with perf on Linux.
...
The flame graph, invented by Brendan D. Gregg, is a wonderful technique to visualize performance statistics. `perf.data` was previously generated by perf, can be used to create a flame graph. You first need to clone the flame graph's repository from GitHub:
...
> Process, thread-pool, and process-pool execution models receive 5, 7, and 10 points, respectively. Total 32 points.
...
* Inspect the data structure of the pool in the thread-pool and process-pool implementations. Is it possible that their destructors include a flaw? If so, why? what can be done to make things better?
